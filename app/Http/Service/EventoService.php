...
class EventoService
{
    public static function SqlEvento($DBTMP ="")
    {
        ...
        $sql = "select DISTINCT ev.id, ev.data, ifNull(pr.nome, 'PROGRAMA EXCLUÍDO') as programa, em.nome as emissora, '' as form_pai,
                 ...
                 when pr.transcricao_prioridade  = 'Normal' then 2
                  ...
                  em.transcricao_url as path, em.transcricao_url2 as alt_path , '' as blnk, '' as tempo_h
                    ...
                    LEFT JOIN eventos_arquivos ea ON ev.id = ea.id_evento
                    LEFT JOIN eventos_arquivos_palavras eap ON ea.id = eap.id_evento_arquivo
                    LEFT JOIN {$DB_MIDIACLIP}.dicionario_tags dic ON eap.id_dicionario_tag = dic.id AND dic.ativo = 1";
    ...
    public static function getListCliente($id_evento)
    {
        ...
        . "     `c`.`id` AS `id`, "
        . "     `c`.`nome` AS `nome` "
        . "     FROM `eventos_arquivos_palavras` AS `eap` "
        . "         INNER JOIN ".$DB_MIDIACLIP.".`cliente` AS `c` "
        . "             ON `eap`.`id_cliente` = `c`.`id` "
        . "         WHERE `eap`.`id_evento` = ". $id_evento. " ORDER BY `c`.`nome` ";
