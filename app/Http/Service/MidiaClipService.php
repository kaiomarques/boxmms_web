...
class MidiaClipService{
        ...
        private static function ComplementoFiltro(string $prefixo) {
            return " and ifNull(". $prefixo ."status, 1 ) = 1 ";
        }
        ...
        public static function getListTopico($id_cliente){
            ...
            $complemento_filtro = SELF::ComplementoFiltro("`cc`.");

            $sql = "SELECT "
                ."`cc`.`id` AS `id`, "
                ."convert(`cc`.`nome` using utf8) AS `nome`, "
                ."`cc`.`id_cliente` AS `id_cliente`, "
                ."0 AS `pai` "
                ."FROM ".$DB_MIDIACLIP.".`classes_cliente` AS `cc` "
                ."    INNER JOIN ".$DB_MIDIACLIP.".`cliente` AS `c` ON `cc`.`id_cliente` = `c`.`id_registro_importado` AND `c`.`ativo` = 1 "
                ."WHERE `c`.`id` = " . $id_cliente . SELF::ComplementoFiltro("`cc`.")
                ."    AND `cc`.`id_pai` IS NULL AND `cc`.`ativo` = 1 "
                ."    ORDER BY `cc`.`nome`, `cc`.`ordem` ";
            ...
            for ($i = 0; $i < count($dt); $i++)
            {
                ...
                $sqltmp = " select id, convert(nome using utf8) as nome , id_cliente, 0 as pai from "
                     ...
                     $id_cliente. " and id_pai = " . $dr->id. SELF::ComplementoFiltro(""). " order by ordem ";
