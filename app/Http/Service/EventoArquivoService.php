...
class EventoArquivoService
{
        ...
        for ($i = 0; $i < count($lista); $i++) {
            ...
            for ($z = 0; $z < count($objects); $z++) {
                ...
                if ($object->start_time < $inicio_corte) {
                    continue;
                } //Legenda abaixo do tempo, não precisamos pegar..
    ...
    public static function geraMateriaByArquivo($id, $id_materia, $id_materia_frags, $json_data, $clientes, Request $request)
    {
        ...
        // $registro->texto = utf8_decode($obj_json->conteudo);
        // $registro->texto_html = utf8_decode($obj_json->conteudo);
        // $registro->sinopse = utf8_decode($obj_json->sinopse);
        // $registro->sinopse_html = utf8_decode($obj_json->sinopse);

        $registro->titulo = $obj_json->titulo;
        $registro->texto = $obj_json->conteudo;
        $registro->texto_html = $obj_json->conteudo;
        $registro->sinopse = $obj_json->sinopse;
        ...
        if (property_exists($obj_materia_frags, "clientes")) {
            ...
            for ($i =0; $i < count($obj_clientes); $i++) {
                ...
                } elseif ($oAssociacao->id_tipo_entidade == "22") {
                    $classificacao .=  "xsubentidade";
                } elseif ($oAssociacao->id_tipo_entidade  == "23") {
    ...
    public static function obtemPalavrasArquivoPai($id_arquivo_pai)
    {
        $DB_MIDIACLIP = config("app.DB_MIDIACLIP");
           
        $sql = "select 
        dt.id_registro_importado as id, 
        dt.id as idMMS,
        c.id as clienteId
        FROM eventos_arquivos eaf
            INNER JOIN eventos e
                ON eaf.id_evento = e.id
            INNER JOIN eventos_arquivos eap
                ON e.id_evento_pai = eap.id_evento
            INNER JOIN eventos_arquivos_palavras AS eapp
                ON eap.id = eapp.id_evento_arquivo
            INNER JOIN ".$DB_MIDIACLIP.".dicionario_tags AS dt
                ON eapp.id_dicionario_tag = dt.id
            INNER JOIN ".$DB_MIDIACLIP.".associacao_cadastros ac
                ON dt.id = ac.id_filho
            INNER JOIN ".$DB_MIDIACLIP.".cliente AS c
                ON ac.id_pai = c.id_registro_importado
            WHERE eaf.id =".$id_arquivo_pai;
    ...
    public static function obtemTodasPalavrasArquivoPai($id_arquivo_filho, $evento_pai, $id_arquivo_pai, $inicio, $fim)
    {
        $id_join = $id_arquivo_pai;
        $arquivo_pai = \App\EventosArquivos::find($id_join);
        ...
        $sql = "select "
               ...
               ." dt.id as idMMS, "
               ." c.id as clienteId "
               ." from eventos_arquivos_palavras AS eapp "
               ."     inner join ".$DB_MIDIACLIP.".dicionario_tags AS dt "
               ."         on eapp.id_dicionario_tag = dt.id "
               ."     inner join ".$DB_MIDIACLIP.".cliente AS c "
               ."         on eapp.id_cliente = c.id "
               ."     where eapp.id_evento_arquivo = " . $id_arquivo_pai
               ."         and eapp.tempo_seg >= " . $hora_inicio_seg
               ."         and eapp.tempo_seg <= " . $hora_fim_seg ;
    ...
    public static function gerarArquivoCorte($evento_pai, $id_arquivo_pai, $inicio, $fim, $titulo = "")
    {
        ...
        $ffmeg_retorno = FFmpegService::gerarCorte(
            $caminho_arquivo_pai,
            $path_evento.DIRECTORY_SEPARATOR.$arquivo_novo,
            $inicio,
            $fim
        );
