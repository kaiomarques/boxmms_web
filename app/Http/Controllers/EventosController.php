...
class EventosController extends Controller
{
    ...
    public function index2(Request $request)
    {
        ...
        $id_praca = $request->input("id_praca");
        ...
        if ($id_programa != "" && is_numeric($id_programa) && $id_programa != -1) {
            $filtro .= " and ev.id_programa = ". $id_programa;
        }
            
        if ($id_emissora != "" && is_numeric($id_emissora) && $id_emissora != -1) {
            $filtro .= " and ev.id_emissora = ". $id_emissora;
        }
            
        if ($id_praca != "" && is_numeric($id_praca) && $id_praca != -1) {
            $filtro .= " and em.id_praca = ". $id_praca;
        }

        if ($id_cliente != "" && is_numeric($id_cliente) && $id_cliente != -1) {
            $filtro .= " and exists ( select sub.id from eventos_arquivos_palavras sub INNER JOIN  {$DB_MIDIACLIP}.cliente c ON sub.id_cliente = c.id AND c.ativo = 1 where sub.id_cliente= ". $id_cliente;
            ...
            $filtro .= " and sub.id_evento = ev.id )  ";
        }
        ...
        if (isset($veiculo_id) && is_numeric($veiculo_id) && $veiculo_id != -1) {
            $veiculoId = $veiculo_id;
        }

        $filtro .= " and ({$veiculoId} IS NULL OR pr.id_meio_comunicacao = {$veiculoId} ) ";
        $filtro .= " and ({$veiculoId} IS NULL OR em.id_veiculo = {$veiculoId}) ";

        $clienteId = "null";

        if (isset($id_cliente) && !empty($id_cliente) && is_numeric($id_cliente) && $id_cliente != -1) {
            $clienteId = $id_cliente;
        }

        $sql = \App\Http\Service\EventoService::SqlEvento($DB_MIDIACLIP, $id_cliente)." where 1 = 1  and ({$clienteId} IS NULL OR dic.id IS NOT NULL) and ifNull(ev.tipo, 'tipo') = 'pai' ". $filtro . " order by ev.id desc "; //
