...
class EventosArquivosController extends Controller
{
    ...
    public function index2(Request $request)
    {
        ...
        if (isset($veiculo_id) && is_numeric($veiculo_id) != "") {
            $filtro .= " and (pr.id_meio_comunicacao IS NULL OR pr.id_meio_comunicacao = " . $veiculo_id . ") ";
        }
    ...
    public function gerarRecorte(Request $request)
    {
        ...
        if ($TRANSCRICAO_BUSCA_EXTERNA) {
            $palavras = EventoArquivoService::obtemTodasPalavrasArquivoPai($novo->id, $evento_pai, $id_arquivo_pai, $inicio, $fim);
            $canais = [];
            $palavrasChave = [];

            foreach ($palavras as $palavra) {
                if ($palavra->id != null) {
                    array_push($canais, $palavra);
                }
            }
            
            foreach ($palavras as $palavra) {
                if ($palavra->id == null) {
                    array_push($palavrasChave, $palavra);
                }
            }
            ...
            if (count($palavrasChave) > 0) {
                foreach ($palavrasChave as $palavra) {
                    ...
                    $vinculo->id_cliente = $palavra->clienteId;
                    $vinculo->cita_diretamente = 1;
                    $vinculo->id_dicionario_tag =$palavra->idMMS;
                    $vinculo->status = 1;
        ...
        if (isset($resultados) && count($resultados) > 0) {
            for ($i = 0; $i < count($resultados); $i++) {
                $obj = $resultados[$i];
                \App\Http\Dao\EventosArquivosPalavrasDao::salvar($obj, $obj->palavra, $obj->operador, $obj->id_dicionario_tag);
            }
        }
