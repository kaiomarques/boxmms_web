...
class MateriaRascunhoController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (isset($veiculo_id) && trim($veiculo_id) != "") {
            $filtro .= " and (pro.id_meio_comunicacao IS NULL OR pro.id_meio_comunicacao = " . $veiculo_id . ") ";
            $filtro .= " and (emi.id_veiculo IS NULL OR emi.id_veiculo = " . $veiculo_id . ") ";
        }
        //p.*, '' as blnk, '' as user_link
        $sql = "select p.id, p.id_projeto, p.data, p.titulo, p.id_projeto,p.id_operador, p.data_cadastro,"
                        . " p.status, p.id_materia_radiotv_jornal, p.ids_arquivos,  "
                        . " p.cliente_list, p.id_programa, p.dia, pro.nome as programa_nome, emi.nome as emissora_nome , "
                        /*. " us.nome as nome_operador, "*/
                        . " '' as blnk "
        ...
        if (isset($itens) && count($itens) > 0) {
            for ($i = 0; $i < count($itens); $i++) {
                ...
                if (isset($item->cliente_list) && strpos($item->cliente_list, ',') !== false) {
                    $list = array_unique(explode(',', $item->cliente_list));
                    $clientes_total = count($list);

                    if ($clientes_total > 1) {
                        $item->cliente_list = $list[0] . " +" . $clientes_total . " cliente(s).";
                    }
                } elseif (!isset($item->cliente_list) || $item->cliente_list == "") {
                    $item->cliente_list = "-";
                }
            }
        }
    ...
    public function index2(Request $request)
    {
        ...
        if (isset($veiculo_id) && is_numeric($veiculo_id) != "") {
            $filtro .= " and (pro.id_meio_comunicacao IS NULL OR pro.id_meio_comunicacao = " . $veiculo_id . ") ";
            $filtro .= " and (emi.id_veiculo IS NULL OR emi.id_veiculo = " . $veiculo_id . ") ";
        }
