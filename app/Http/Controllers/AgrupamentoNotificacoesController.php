...
class AgrupamentoNotificacoesController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (trim($id_evento_status) != "" && $id_evento_status != "-1" && $id_evento_status != "0") {
            $filtro .= " and eve.status = " . $id_evento_status;
        }
        ...
        if ($status == "" && $status_evento == "") {
            ...
            $filtro .= " and ifNull(eve.status, 1 ) = ".$status_evento."   ";
        }
    ...
    {              
        ...
        $palavrasChave = new QueryNotificacoesPorPalavrasChave($DB_MIDIACLIP);
        $union = new QueryNotificacoesUnion($original, $palavrasChave);
        ...
        $iniciandoEm = isset($dt_inicio) && !empty($dt_inicio) ? (new DateTime($dt_inicio))->format("Y-m-d") : date("Y-m-d");
        $terminandoEm = isset($dt_fim) && !empty($dt_fim) ? (new DateTime($dt_fim))->format("Y-m-d") : date("Y-m-d");
            
        $union
            ->iniciandoEm($iniciandoEm)
            ->terminandoEm($terminandoEm)
            ->doPrograma($id_programa)
            ...
            ->comIdsMaioresQue($acima_de)
            ->daMidia($veiculo_id); 
        $countSql = $union
            ->count(true)
            ->sql();
        $total_itens = $this->executeScalar($countSql);
        ...
        if ($order_type == "desc") 
            $union->orderByDesc($order);
        else 
            $union->orderByAsc($order);
        $sql = $union
            ->count(false)
