...
<template>
  <div :dir="dir" class="v-select" :class="stateClasses">
    ...
    <!-- v-on="scope.search.events" -->
        <slot name="search" v-bind="scope.search" v-on="scope.search.events">
          <input class="vs__search" v-bind="scope.search.attributes" >
        </slot>
      </div>
                 ...
                 for ( var i = 0; i < this.options.length; i++ ){
                      if ( this.options[i].value == option ){
                         //emit
                           ...
                           this.$emit('update:label_selected', this.options[i][this.label]);
